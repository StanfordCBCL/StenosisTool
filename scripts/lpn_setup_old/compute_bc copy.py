...
# Last Modified: Monday, 31st October 2022 2:46:19 pm
...
############################
def compute_total_resistance(cur_node: LPN.Node, use_coeff: bool, solver_results: SolverResults):
    ...
    for child_node in cur_node.children:
        r_p_child = compute_total_resistance(child_node)
        if r_p_child != 0:
...
def compute_lpa_rpa_resistances(base_lpn: LPN, use_coeff = False):
    ''' computes LPA and RPA resistances from each branch
    '''
    vessel_tree = base_lpn.get_vessel_tree()
    ...
    lpa_res = compute_total_resistance_round1(lpa, use_coeff)
    rpa_res = compute_total_resistance_round1(rpa, use_coeff)
...
def construct_tuning_lpn(params: TuneParams, inflow: Inflow0D, centerlines: Centerlines, base_lpn: LPN) -> LPN:
    ...
    # Compute MPA
    units = base_lpn.simulation_params['units']
    if units == 'cm':
        coeff = 1
...
def loss_function(results: SolverResults, tune_params: TuneParams, inflow: Inflow0D):
    ...
    Q_RPA_loss =  squared_error(Q_RPA_meas, Q_RPA_sim)
    maxPAP_loss = piecewise_error(maxPAP_meas[0], maxPAP_meas[1], maxPAP_sim)
    return  mPAP_loss , Q_RPA_loss, .1 * mse_loss,  maxPAP_loss, (mPAP_sim, Q_RPA_sim, maxPAP_sim ,mPAP_meas, Q_RPA_meas, maxPAP_meas)
...
def calc_rcrs(results, inlet, area_file):
    ''' Splits RCRS '''
...
def main(args: Namespace):
    ...
    for lpn_dirname in args.lpn_dirs:
        
        lpn_dir = LPNDir(lpn_dirname)
        lpn_dir.check_complete()
        
        base_lpn = LPN()
        base_lpn.read_lpn_file(str(lpn_dir.lpn_path))
        print(f"Optimizing model {str(lpn_dir.lpn_path)}...")
        
        if not lpn_dir.tuning_dir.exists():
            lpn_dir.tuning_dir.mkdir()
            
        results_file = lpn_dir.tuning_dir / 'optimize_results.npy'
        if not args.force and results_file.exists():
                print(f'Result file found. Skipping optimization for {lpn_dirname}')
                continue
        ...
        inflow = Inflow0D.from_file(str(lpn_dir.inflow_file))

        # load centerlines file
        centerlines = Centerlines()
        centerlines.load_polydata(str(lpn_dir.centerlines))
                
        # set up solver file
        tuning_lpn = construct_tuning_lpn(tuning_params, inflow, centerlines, base_lpn)
        ...
        lpa['zero_d_element_values']['stenosis_coefficient'] = 0
        rpa = tuning_data.get_vessel(4)
        rpa['zero_d_element_values']['R_poiseuille'] =  rpa_r
        rpa['zero_d_element_values']['stenosis_coefficient'] = 0
        ...
        bcs = calc_rcrs(results=results_dict, 
                  inlet=inlet,
                  area_file=area_file)
        bcs.write_rcrt_file(solver_dir)
...
if __name__ == '__main__':
    ...
    tool = ToolParser(desc = 'Tunes a model')
    
    # dev params
    tool.parser.add_argument('-f', dest = 'force', action = 'store_true', default = False, help = 'Whether to run an optimization if one already exists')
    tool.parser.add_argument('-int', action = 'store_true', default = False, help = 'intermediate results')
    tool.parser.add_argument('-sens_test', action = 'store_true', default = False, help = 'whether to run sensitivity tests or not')
