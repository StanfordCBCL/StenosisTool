...
if __name__ == '__main__':
    ...
    #! Can change this to 3 sections for a test set too
    train_len = int(.8 * len(sim_dataset))
    val_len = len(sim_dataset) - train_len
    train_dataset, val_dataset = tdata.random_split(sim_dataset, lengths=[train_len, val_len])
    
    
    train_loader = tdata.DataLoader(train_dataset, batch_size = 128, shuffle = True)
    val_loader = tdata.DataLoader(val_dataset, batch_sampler=128, shuffle = True)
    
    # retrieve first value of Dataset for sizes
    input_data, output_data = sim_dataset[0]
    # Arbitrary
    nnmodel = BasicNN(input_neurons=len(input_data), output_neurons=len(output_data), hidden_layers=10, neurons_per_layer=30)
