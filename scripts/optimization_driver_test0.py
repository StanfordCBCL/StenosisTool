...
def compute_lpa_rpa_resistance_clinical(solver: Solver0D, solver_results: SolverResults):
    ...
    '''
    vessel_tree = solver.get_vessel_tree()
    
    rpa, lpa = get_lpa_rpa(vessel_tree)
    
    # compute rpa resistance
    rpa_vess_id = rpa.vess_id[-1]
    rpa_rez = solver_results.only_last_cycle(solver_results.vessel_df('V' + str(rpa_vess_id)), solver.inflow.tc)
    time = np.array(rpa_rez['time'])
    rpa_p_in = np.trapz(np.array(rpa_rez['pressure_in']), time) / (time[-1] - time[0])
    rpa_Q_in =np.trapz(np.array(rpa_rez['flow_in']), time) / (time[-1] - time[0])
    max_pd = 0
    for node in solver.tree_bfs_iterator(vessel_tree):
        if node.get_type() == 'outlet':
            tmp_rez = solver_results.only_last_cycle(solver_results.vessel_df('V' + str(rpa_vess_id)), solver.inflow.tc)
            tmp_p_out = np.trapz(np.array(tmp_rez['pressure_out']), time) / (time[-1] - time[0])
            pd = rpa_p_in - tmp_p_out
            if pd > max_pd:
                max_pd = pd
                
    rpa_r = max_pd / rpa_Q_in

    # Compute lpa resistance
    lpa_vess_id = lpa.vess_id[-1]
    lpa_rez = solver_results.only_last_cycle(solver_results.vessel_df('V' + str(lpa_vess_id)), solver.inflow.tc)
    time = np.array(lpa_rez['time'])
    lpa_p_in = np.trapz(np.array(lpa_rez['pressure_in']), time) / (time[-1] - time[0])
    lpa_Q_in =np.trapz(np.array(lpa_rez['flow_in']), time) / (time[-1] - time[0])
    max_pd = 0
    for node in solver.tree_bfs_iterator(vessel_tree):
        if node.get_type() == 'outlet':
            tmp_rez = solver_results.only_last_cycle(solver_results.vessel_df('V' + str(lpa_vess_id)), solver.inflow.tc)
            tmp_p_out = np.trapz(np.array(tmp_rez['pressure_out']), time) / (time[-1] - time[0])
            pd = lpa_p_in - tmp_p_out
            if pd > max_pd:
                max_pd = pd
                
    lpa_r = max_pd / lpa_Q_in
...
def main(args: Namespace):
    ...
    for solver_dir in args.solver_dirs:
        ...
        lpa_r, rpa_r = compute_lpa_rpa_resistance_clinical(dummy_solver, dummy_rez)
        print('Clinical dP:', d2m(inflow.mean_inflow * 1/(1/lpa_r + 1/rpa_r)), 'LPA_R:',lpa_r, 'RPA_R:',rpa_r)
