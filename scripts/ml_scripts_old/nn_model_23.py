...
class BasicNN(nn.Module):
    ...
    def __init__(self, input_neurons, output_neurons, hidden_layers, neurons_per_layer):
        ...
        # input for 1st gen
        self.input1_layer = nn.Linear(input_neurons, neurons_per_layer )
        self.relu = nn.ReLU()
        ...
        if hidden_layers < 1:
            ...
            for i in range(hidden_layers):
                self.hidden1.append(nn.Linear(neurons_per_layer, neurons_per_layer))
                self.hidden1.append(self.relu)
        self.output1_layer = nn.Linear(neurons_per_layer, 6)

        # input for 2nd gen
        self.input2_layer = nn.Linear(input_neurons + 6, neurons_per_layer )
        self.hidden2 = nn.Sequential()
        if hidden_layers < 1:
            raise ValueError('hidden layers must be > 0')
        else:
            for i in range(hidden_layers):
                self.hidden2.append(nn.Linear(neurons_per_layer, neurons_per_layer))
                self.hidden2.append(self.relu)
        
        self.output2_layer = nn.Linear(neurons_per_layer, 12)
        
        # input for 3nd gen
        self.input3_layer = nn.Linear(input_neurons + 6 + 12, neurons_per_layer )
        self.hidden3 = nn.Sequential()
        if hidden_layers < 1:
            raise ValueError('hidden layers must be > 0')
        else:
            for i in range(hidden_layers):
                self.hidden3.append(nn.Linear(neurons_per_layer, neurons_per_layer))
                self.hidden3.append(self.relu)
        
        self.output3_layer = nn.Linear(neurons_per_layer, 30)
        

    
    def forward(self, x):
        tmp = x
        tmp = self.input1_layer(tmp)
        tmp = self.relu(tmp)
        tmp = self.hidden1(tmp)
        tmp1 = self.output1_layer(tmp)
        
        tmp = torch.concat([x, tmp1], dim = 1)
        tmp = self.input2_layer(tmp)
        tmp = self.relu(tmp)
        tmp = self.hidden2(tmp)
        tmp2 = self.output2_layer(tmp)
        
        tmp = torch.concat([x, tmp1, tmp2], dim = 1)
        tmp = self.input3_layer(tmp)
        tmp = self.relu(tmp)
        tmp = self.hidden3(tmp)
        tmp3 = self.output3_layer(tmp)
        
        return torch.concat([tmp1, tmp2, tmp3], dim = 1)
