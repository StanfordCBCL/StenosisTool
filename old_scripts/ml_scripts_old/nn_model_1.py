...
def normalization(output):
    ...
    for i in range(len(output[0])):
        ...
        revert_map.append({'max': max, 'min': min})
    
    return output, revert_map

def revert(output, map_back):
    for i in range(len(output[0])):
        output[:, i] = (output[:, i] * (map_back[i]['max'] - map_back[i]['min'])) + map_back[i]['min']
    return output
...
if __name__ == '__main__':
    ...
    trainer = pl.Trainer( max_epochs=100, accelerator="gpu", default_root_dir=cur_results_folder, callbacks=[checkpoint_callback, early_stop], logger = csv_logger, log_every_n_steps=5)#, fast_dev_run=True)
